...
export function BillSidebar({ 
  ...
  return (
    <aside className="sidebar-panel sticky top-28 h-[calc(100vh-140px)]">
      <div className="flex justify-between items-center mb-6">
        <h3 className="text-sm font-black uppercase tracking-widest">{title}</h3>
        <button
          onClick={onReset}
          className="text-[10px] font-black text-muted-foreground tracking-widest border border-border px-3 py-1 rounded-md hover:bg-destructive hover:text-destructive-foreground hover:border-destructive transition-all"
        >
          RESET
        </button>
      </div>
      ...
      <div className="session-card">
        <div className="grid grid-cols-2 gap-3">
          <div className="flex flex-col gap-0.5">
            <span className="text-[9px] text-muted-foreground font-extrabold uppercase tracking-widest">
              {sessionInfo.label1}
            </span>
            <span className="text-xs text-foreground font-extrabold">{sessionInfo.value1}</span>
          </div>
          <div className="flex flex-col gap-0.5">
            <span className="text-[9px] text-muted-foreground font-extrabold uppercase tracking-widest">
              {sessionInfo.label2}
            </span>
            <span className="text-xs text-foreground font-extrabold">{sessionInfo.value2}</span>
          </div>
        </div>
      </div>
          ...
          categories.map(cat => {
            ...
            return (
              <div key={cat} className="mb-4">
                <span className="block text-xs font-extrabold text-primary mb-2">{cat}</span>
                {items.map((item, idx) => {
                  ...
                  return (
                    <div key={`${item.name}-${idx}`} className="bill-line">
                      ...
                      <X 
                        className="w-4 h-4 ml-2 text-muted-foreground cursor-pointer hover:text-destructive transition-colors"
      ...
      <div className="border-t border-border pt-6 mt-4">
        ...
        <button onClick={onFinalize} className="btn-primary w-full" disabled={bill.length === 0}>
